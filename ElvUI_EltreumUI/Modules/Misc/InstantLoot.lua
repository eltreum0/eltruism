...
--wishlist popup
local WishlistItemFrame = CreateFrame("Frame", "EltruismWishlistItem", _G.UIParent)
...
local function InstantLoot(_, event,_, arg2)
	if event == "LOOT_BIND_CONFIRM" and (E.Mists or E.TBC or E.Wrath or E.Classic) then
		...
		if E.db.ElvUI_EltreumUI.loot.fastlootfilter then
			if event == 'LOOT_BIND_CONFIRM' then
				...
				for i = GetNumLootItems(), 1, -1 do
					...
					if isQuestItem then
						LootSlot(i)
					elseif lootQuality and lootQuality >= tonumber(E.db.ElvUI_EltreumUI.loot.fastlootquality) then
						LootSlot(i)
