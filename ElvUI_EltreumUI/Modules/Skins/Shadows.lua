...
--seems to throw a LinkAchievementInWhisper in 12.0.7
...
local function BlizzShadows(arg)
	...
	if E.private.skins.blizzard.enable then
		if (arg == "Blizzard_SwingTimer") or IsAddOnLoaded("Blizzard_SwingTimer") and E.private.skins.blizzard.swingTimer then
			for _, frame in _G.next, { _G.SwingTimerMainHandFrame, _G.SwingTimerOffHandFrame, _G.SwingTimerRangedFrame } do
				if frame and frame.StatusBar then
					if frame.StatusBar.backdrop then
						frame.StatusBar.backdrop:CreateShadow(E.db.ElvUI_EltreumUI.skins.shadow.length)
						ElvUI_EltreumUI:ShadowColor(frame.StatusBar.backdrop.shadow)
					else
						frame.StatusBar:CreateShadow(E.db.ElvUI_EltreumUI.skins.shadow.length)
						ElvUI_EltreumUI:ShadowColor(frame.StatusBar.shadow)
					end
				end
			end
		end
	...
	if E.db.ElvUI_EltreumUI.skins.shadow.enable and not E.db.ElvUI_EltreumUI.borders.universalborders and not E.db.ElvUI_EltreumUI.skins.shadow.universalshadows then
		...
		if E.db.ElvUI_EltreumUI.skins.shadow.unitframes then
			if E.private.unitframe.enable then
					...
					if E.db.unitframe.units.focustarget.enable and E.db.unitframe.units.focustarget.portrait.enable and not E.db.unitframe.units.focustarget.portrait.overlay then
						if _G["ElvUF_FocusTarget"] and not _G["ElvUF_FocusTarget"].shadow then
							if not (E.db.ElvUI_EltreumUI.borders.focustargetborder and E.db.ElvUI_EltreumUI.borders.borders) then
								...
								if E.db.unitframe.units.focustarget.orientation == "RIGHT" then
									_G["ElvUF_FocusTarget"].shadow:SetPoint("TOPRIGHT", _G["ElvUF_FocusTarget"].Portrait,"TOPRIGHT", E.db.ElvUI_EltreumUI.skins.shadow.length, E.db.ElvUI_EltreumUI.skins.shadow.length)
									if E.db.unitframe.units.focustarget.infoPanel.enable and _G["ElvUF_FocusTarget"].USE_INFO_PANEL then
				...
				--player castbar
				if E.db.unitframe.units.player.enable and E.db.unitframe.units.player.castbar.enable and E.db.unitframe.units.player.castbar.overlayOnFrame == "None" and not (E.db.ElvUI_EltreumUI.borders.playercastborder and E.db.ElvUI_EltreumUI.borders.borders) and E.db.unitframe.units.player.castbar.icon then
					if _G["ElvUF_Player_CastBar"].shadow then
				...
				--target castbar
				if E.db.unitframe.units.target.enable and E.db.unitframe.units.target.castbar.enable and E.db.unitframe.units.target.castbar.overlayOnFrame == "None" and not (E.db.ElvUI_EltreumUI.borders.targetcastborder and E.db.ElvUI_EltreumUI.borders.borders) and E.db.unitframe.units.target.castbar.icon then
					if _G["ElvUF_Target_CastBar"].shadow then
				...
				--focus castbar
				if E.db.unitframe.units.focus.enable and E.db.unitframe.units.focus.castbar.enable and E.db.unitframe.units.focus.castbar.overlayOnFrame == "None" and E.db.unitframe.units.focus.castbar.icon and E.db.unitframe.units.focus.enable then
							...
							if button and not button.shadow then
								if button.db.power.enable then
									...
									elseif button.db.power.width == "offset" then
										...
										button:CreateShadow(E.db.ElvUI_EltreumUI.skins.shadow.length)
										if E.db.ElvUI_EltreumUI.unitframes.infopanelontop and E.db.ElvUI_EltreumUI.unitframes.infopanelontopallframes and button.db.infoPanel.enable and button.USE_INFO_PANEL then
											button.shadow:ClearAllPoints()
											button.shadow:SetPoint("BOTTOMLEFT", button.Health,"BOTTOMLEFT", -E.db.ElvUI_EltreumUI.skins.shadow.length, -E.db.ElvUI_EltreumUI.skins.shadow.length)
											button.shadow:SetPoint("BOTTOMRIGHT", button.Health,"BOTTOMRIGHT", E.db.ElvUI_EltreumUI.skins.shadow.length, -E.db.ElvUI_EltreumUI.skins.shadow.length)
		...
		if E.db.ElvUI_EltreumUI.borders.borders and (E.db.ElvUI_EltreumUI.borders.nameplateborders or E.db.ElvUI_EltreumUI.borders.nameplatecastborders) then
			...
			if E.db.ElvUI_EltreumUI.borders.nameplateborders and nameplate.Health then
				...
				if E.db.ElvUI_EltreumUI.borders.classcolor then
					if bordercolor.r then
						nameplate.Health.EltruismNameplateBorder:SetBackdropBorderColor(bordercolor.r, bordercolor.g, bordercolor.b, 1)
					else --fallback because color failed
						nameplate.Health.EltruismNameplateBorder:SetBackdropBorderColor(1, 1, 1, 1)
					end
				else
