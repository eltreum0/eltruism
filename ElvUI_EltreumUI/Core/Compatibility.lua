...
local function ConstructCompatibilityFrame()
	...
	desc:SetText(L["There are many modules from different addons or ElvUI plugins, but several of them are almost the same functionality."] .." " .. format(L["Have a good time with %s!"], select(2,GetAddOnInfo("ElvUI_EltreumUI"))))
	...
	largeTip:SetText("|c"..E:RGBToHex(classcolor.r,classcolor.g,classcolor.b, 'ff')..L["Choose the module you would like to |cff1784d1use|r"].."|r")
	...
	bottomDesc:SetText(E.NewSign ..format(L["If you find the %s module conflicts with another addon, alert me via Discord."], select(2,GetAddOnInfo("ElvUI_EltreumUI"))) .."\n" ..L["You can disable/enable compatibility check via the option in the bottom of [WindTools]-[Information]-[Help]."])
...
function ElvUI_EltreumUI:CheckCompatibility()
	...
	--WindTools
	CheckWindTools(format("%s\n%s", L["Waypoints"], L["Autopin, Time to Arrive"]), format("%s\n%s", L["Waypoints"], L["Autopin"]), "db.ElvUI_EltreumUI.waypoints.waypointetasetting.enable", "private.WT.maps.superTracker.autoTrackWaypoint")
